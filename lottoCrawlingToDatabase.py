...
class LottoDB():
    def get_recent_lottocount(self): # 최신 로또 회차 크롤링 함수(2024년 12월 17일 현재 1150회차)
    ...
    def get_lottoNumber(self, lottocount):  # 로또 회차를 넣으면 해당 회차의 로또당첨결과를 반환하는 함수
    ...
    def lotto_to_db(self):
        lottoNumList = []  # 로또 당첨 결과 레코드가 저장될 빈 리스트
        #{"count":로또추첨회차, "lottoDate":추첨일, "lottoNum1":당첨번호1번, "lottoNum2":당첨번호2번,...."bonusNum":보너스번호}
        for count in range(1, self.get_recent_lottocount()+1):  # 1~1150회까지 반복
            lottoResult = self.get_lottoNumber(count)  # 해당 회차의 로또당첨결과를 반환
